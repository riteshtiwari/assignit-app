...
/**
 * A horizontally scrollable {@link ViewGroup} with items populated from an {@link Adapter}. The
 * ViewFlow uses a buffer to store loaded {@link View}s in. The default size of the buffer is 3
 * elements on both sides of the currently visible {@link View}, making up a total buffer size of 3
 * * 2 + 1 = 7. The buffer size can be changed using the {@code sidebuffer} xml attribute.
 * 
 */
public class ViewFlow extends AdapterView<Adapter> {
	...
	/**
	 * Extra return value from obtainView: tells you whether the item it returned on the last call
	 * was recycled rather than created by the adapter. This is a member because getting a second
	 * return value requires an allocation.
	 */
			...
			case MotionEvent.ACTION_MOVE:
				...
				if (mTouchState == TOUCH_STATE_SCROLLING) {
					...
					if (deltaX < 0) {
						...
						final int availableToScroll = getChildAt(getChildCount() - 1).getRight() - getPaddingRight()
								- getHorizontalFadingEdgeLength() - scrollX - getWidth();
			...
			case MotionEvent.ACTION_MOVE:
				...
				if (mTouchState == TOUCH_STATE_SCROLLING) {
					...
					if (deltaX < 0) {
						...
						final int availableToScroll = getChildAt(getChildCount() - 1).getRight() - getPaddingRight()
								- getHorizontalFadingEdgeLength() - scrollX - getChildWidth();
						if (availableToScroll > 0) {
							scrollBy(Math.min(availableToScroll, deltaX), 0);
						}
					}
	...
	protected void onScrollChanged(int h, int v, int oldh, int oldv) {
		...
		if (mIndicator != null) {
			 ...
			 * Therefore, we need to calculate the perceived horizontal scroll origin here, since we
			 * use a view buffer.
			 */
	...
	private void logBuffer() {
		...
		Log.d("viewflow", "Size of mLoadedViews: " + mLoadedViews.size() + ", Size of mRecycledViews: "
				+ mRecycledViews.size() + ", X: " + mScroller.getCurrX() + ", Y: " + mScroller.getCurrY());
